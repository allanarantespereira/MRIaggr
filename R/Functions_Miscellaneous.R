...
constLatex <- function(directory, filename = NULL, identifier = NULL, param = NULL, tabular = NULL, extra_text = NULL, 
  ...
  if(!is.null(extra_text)){
    validDimension(value1 = unlist(extra_text), value2 = identifier, name1 = "extra_text", name2 = "identifier", type = "length", method = "constLatex")
  }
...
getFileExtention <- function(filename){
  ...
  filename_splitPoint <- strsplit(filename, split = ".", fixed = TRUE)[[1]]
  n.filename_splitPoint <- length(filename_splitPoint)
  
  if( n.filename_splitPoint > 1){
    
    if( n.filename_splitPoint > 2 && filename_splitPoint[n.filename_splitPoint] == "gz"){
      extension.compression <- TRUE
      extension <- paste(filename_splitPoint[(n.filename_splitPoint-1):n.filename_splitPoint], collapse = ".")
      filename <- paste(filename_splitPoint[1:(n.filename_splitPoint-2)], collapse = ".")
    }else{
      extension.compression <- FALSE
      extension <- filename_splitPoint[n.filename_splitPoint]
      filename <- paste(filename_splitPoint[1:(n.filename_splitPoint-1)], collapse = ".")
    }
...
constBlanck <- function(x){ # output a blank of a given size
  
  do.call(paste, c(as.list(rep(" ", x), sep = "")))
  
}

initSpace <- function(character){ # output blank so that character+blank have the same size in the vector
  
  nchar.character <- lapply(character, function(x){nchar(x)})
  nchar_max.character <- max(unlist(nchar.character))
  
  space.character <- sapply(character, constBlanck,  nchar_max.character-nchar(x))
...
initSection_constLatex <- function(directory.plot, names_dirs, param, subsection, label){
  ...
  validDimension(value1 = param, value2 = label, name1 = "param", name2 = "label", type = "length", method = "constLatex")
